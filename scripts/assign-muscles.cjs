// CommonJS so it can resolve playwright from NODE_PATH
...
(async () => {
  ...
  await page.waitForTimeout(3000); // let IndexedDB seed

  // Expose a callback so we can receive the result from a script tag (avoids evaluate context destruction)
  let resolveResult;
  const resultPromise = new Promise(r => { resolveResult = r; });
  await page.exposeFunction('__migrationDone', (data) => resolveResult(data));

  await page.addScriptTag({ content: `
    (function() {
      ${MAPPING_SRC}
      const req = indexedDB.open('peak-os-db');
      req.onerror = () => window.__migrationDone({ error: 'open failed' });
      req.onsuccess = () => {
        const db = req.result;
        const tx = db.transaction('exercises', 'readwrite');
        const store = tx.objectStore('exercises');
        const all = store.getAll();
        all.onerror = () => window.__migrationDone({ error: 'getAll failed' });
        all.onsuccess = () => {
          const exercises = all.result;
          const otherExs = exercises.filter(e => e.primaryMuscle === 'Other');
          const assignedList = [], stillOther = [];
          for (const ex of otherExs) {
            const muscle = assignMuscle(ex.name);
            if (muscle) { store.put({ ...ex, primaryMuscle: muscle }); assignedList.push({ name: ex.name, muscle }); }
            else stillOther.push(ex.name);
          }
          tx.oncomplete = () => window.__migrationDone({ total: exercises.length, otherBefore: otherExs.length, assigned: assignedList.length, assignedList, stillOther });
          tx.onerror = () => window.__migrationDone({ error: tx.error && tx.error.message });
        };
      };
    })();
  ` });
